...
Serviço HTTP de detecção de plágio com o **Sherlock** (Rob Pike & Loki), usado pelo [Jude](https://jude.dcc.ufba.br/) (UFBA Online Judge) para comparar submissões de código.

Wrapper Flask mínimo: recebe as submissões, roda o binário `sherlock` e devolve os pares com percentual de similaridade.
...
Resposta: `{ "results": [ { "id1": "123", "id2": "456", "similarity": 87 }, ... ] }`

## Executando

```bash
docker build -t judeufba/jude-sherlock .
docker run --rm -p 5000:5000 judeufba/jude-sherlock
```

Imagem publicada: [`judeufba/jude-sherlock`](https://hub.docker.com/r/judeufba/jude-sherlock) (CI publica `latest` + short-hash a cada push em `main`).
...
# Documentação original do Sherlock
